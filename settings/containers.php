...
class Page {
	...
	public function set_markup_top( $markup = false )
	{
		if(!$markup) {
            ...
            $this->markup_top .=  '<img src="'.MULTIDEALERURL.'assets/images/logosmall.png" />';
 		} else {
...
class Tab {
	...
	public function __construct ( $title, $id, $page, $section_option_settings = array(), $active_state = FALSE ) 
	{
		...
		$this->page = $page;
		$this->id = $id;
 	    $this->anchor = '<a href="?page=' . $this->page->slug . '&post_type=products&tab=' . $this->id . '" class="nav-tab">' . $this->title . '</a>';
	...
	{
		if($active) {
			return '<a href="?page=' . $this->page->slug . '&post_type=products&tab=' . $this->id . '" class="nav-tab nav-tab-active">' . $this->title . '</a>';
		}
		return $this->anchor;
	}
}
