 ...
 */
add_action('delete_attachment', 'multidealer_mr_delete_resized_images');
function multidealer_mr_image_resize($url, $width = null, $height = null, $crop = true, $align =
    'c', $retina = false)
{
    global $wpdb;
    // Get common vars (func_get_args() only get specified values)
    $common = multidealer_mr_common_info($url, $width, $height, $crop, $align, $retina);
...
function multidealer_mr_common_info($url, $width, $height, $crop, $align, $retina)
{
    ...
    /*
    [path] => /home/autosell/public_html/wp-content/uploads/2017/09
    [url] => http://autosellerplugin.com/wp-content/uploads/2017/09
    [subdir] => /2017/09
    [basedir] => /home/autosell/public_html/wp-content/uploads
    [baseurl] => http://autosellerplugin.com/wp-content/uploads
    [error] => 
    */
    if (preg_match('/\/[0-9]{4}\/[0-9]{2}\/.+$/', $urlinfo['path'], $matches)) {
        ...
        write_log('Upload directory not found (MultiDealer Plugin) - '.__FILE__ .' - Line  '.__LINE__);
         
        if(WP_DEBUG)
        {
           echo 'Warning: Upload directory not found (MultiDealer Plugin) - '.__FILE__ .' - Line  '.__LINE__ ;

        }
        
        if (function_exists('MultiDealer_Show_Notices1'))
        {
            if( current_user_can('editor')) {
              add_action('admin_notices', 'MultiDealer_Show_Errors1');
             }
         } 
        ...
        $file_path = ABSPATH . str_replace(dirname($_SERVER['SCRIPT_NAME']) . '/', '',
            strstr($pathinfo['path'], $uploads_dir));
...
function multidealer_mr_delete_resized_images($post_id)
{
    ...
    foreach ($resized_images as $dims) {
        ...
        $file = $wp_upload_dir['basedir'] . '/' . $pathinfo['dirname'] . '/' . $pathinfo['filename'] .
            '-' . $dims . '.' . $pathinfo['extension'];
